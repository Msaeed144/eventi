...
function Article() {
  ...
  const [ddrcodeFocused, setDdrcodeFocused] = useState(false);
...
const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
if (editingId) {
          setAllArticles((prev) => prev.map((it) => (it.id === editingId ? { ...it, ...article } : it)));
        } else {
          const newItem: ArticleType = { id: uuidv4(), ...article };
          setAllArticles((prev) => [...prev, newItem]);
        }
...
const handleEdit = (item: ArticleType) => {
       ...
       const handleDelete = (id: string) => {
        setAllArticles((prev) => prev.filter((it) => it.id !== id));
      };
  return (
    <div>
                      ...
                      >
                        <div className="w-full flex justify-between">
                          <div>
                            ...
                            <div className="flex mb-1.5">
                              <p>کد DDR:</p>
                              <p className="font-semibold mr-1">{item.ddrcode}</p>
                            </div>
                            <div className="flex mb-1.5">
                              <p>توضیحات:</p>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="w-full grid grid-cols-12 gap-x-11 gap-y-[30px]">
              ...
              <div className="relative pt-3 col-span-6">
               ...
               <div className="relative pt-3 col-span-6">
                <input
                  onFocus={() => setDdrcodeFocused(true)}
                  onBlur={() => setDdrcodeFocused(false)}
                  onChange={changeHandler}
                  value={article.ddrcode}
                  className="w-full focus:border-2 focus:border-primaryColor outline-none noSpinner p-4 rounded-[8px] h-12 border border-strokeColor"
                  type="text"
                  name="ddrcode"
                  id="ddrcode"
                ...
                <label
                  htmlFor="ddrcode"
                  className={`cursor-text absolute right-[15px] text-[#667085] bg-white px-1 transition-all duration-200 ${
                    ddrcodeFocused || article.ddrcode
                      ? "translate-y-[-10px] text-[11px] font-bold text-primaryColor"
                      : "translate-y-[14px] text-[14px] font-medium"
                  }`}
                >
